...
class ProductProduct(models.Model):
    ...
    value_usd_svl = fields.Float(
        compute="_compute_value_usd_svl", compute_sudo=True, company_dependent=True
    )
    ...
    def _compute_value_usd_svl(self):
        ...
        groups = self.env["stock.valuation.layer"].read_group(
            domain, ["value_usd:sum", "quantity:sum"], ["product_id"]
        )
        products = self.browse()
        for group in groups:
            product = self.browse(group["product_id"][0])
            product.sudo().with_company(company_id.id).value_usd_svl = (
                self.env.company.currency_id_dif.round(group["value_usd"])
            )
            products |= product
    ...
    def _set_standard_price_usd(self):
        for rec in self:
            company_id = self.env.company
            if (
                rec.with_company(company_id.id).standard_price_usd
                and rec.categ_id.property_valuation == "manual_periodic"
            ):
                if rec.with_company(company_id.id).standard_price_usd > 0:
                    tasa = self.env.company.currency_id_dif
                    if tasa:
                        rec.sudo().with_company(company_id.id).standard_price = (
                            rec.standard_price_usd / tasa.rate
                        )
    ...
    def _compute_standard_price_usd(self):
        for rec in self:
            tasa = self.env.company.currency_id_dif
            if tasa:
                rec.standard_price_usd = rec.standard_price * tasa.rate
    ...
    def _run_fifo(self, quantity, company):
        ...
        for candidate in candidates:
            ...
            candidate_unit_cost_usd = (
                candidate.remaining_value_usd / candidate.remaining_qty
            )
            new_standard_price = candidate_unit_cost
            new_standard_price_usd = candidate_unit_cost_usd
            value_taken_on_candidate = qty_taken_on_candidate * candidate_unit_cost
            value_taken_on_candidate_usd = (
                qty_taken_on_candidate * candidate_unit_cost_usd
            )
            value_taken_on_candidate = candidate.currency_id.round(
                value_taken_on_candidate
            )
            new_remaining_value = candidate.remaining_value - value_taken_on_candidate
            new_remaining_value_usd = (
                candidate.remaining_value_usd - value_taken_on_candidate_usd
            )
            candidate_vals = {
            ...
            if float_is_zero(
                ...
                if float_is_zero(
                    ...
                    new_standard_price_usd = (
                        next_candidates
                        and next_candidates[0].unit_cost_usd
                        or new_standard_price_usd
                    )
                break
        ...
        for product in self:
            prices[product.id] = (
                product[
                    (
                        price_type
                        if not price_type == "standard_price"
                        else "standard_price_usd"
                    )
                ]
                or 0.0
            )
            if price_type == "standard_price":
                prices[product.id] = (
                    prices[product.id] * self.env.company.currency_id_dif.rate
                )
    ...
    def _compute_list_price_usd(self):
        for rec in self:
            if rec.currency_id_dif:
                rec.list_price_usd = rec.lst_price * rec.currency_id_dif.rate
